...
export async function setupApp (storage, host, port) {
  const app = new Koa()
  ...
  router.head('/v2/:org/:name/blobs/:digest', async (ctx) => {
    ...
    if (size !== null) {
      ...
      ctx.set('Content-Type', 'application/vnd.docker.image.rootfs.diff.tar.gzip') // TODO: set to real? does it matter?
      ctx.status = 200
    } else {
