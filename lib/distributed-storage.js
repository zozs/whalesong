...
import { getHyperbee, getNewHyperbee, getSettingsHyperbee, getIpfsClient, setup, shutdown } from './distributed-clients.js'
...
class DistributedStorage {
  ...
  async init () {
    ...
    // check if the list of subscribed feeds contains any writeable feed, if not, create a new one.
    if (this.getMyPubKey() === null) {
      // now initalize a hyperbee (our own, writable)
      const { key, bee } = await getNewHyperbee()
      this.hyperbees.set(key, bee)
      await this.settings.addSubscribedFeed(key)
    }
