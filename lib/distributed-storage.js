...
import { getHyperbee, getOwnHyperbee, getIpfsClient, setup } from './distributed-clients.js'
...
class DistributedStorage {
  ...
  async getManifest (org, name, tagOrDigest) {
    let manifests = this.manifestStore(org)
    if (!manifests) {
      ...
      debug('storing reference to hyperbee for key %s', key)
  ...
  async putManifest (org, name, tag, data) {
    ...
    const manifests = this.manifestStore(org)
    if (manifests) {
