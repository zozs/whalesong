...
class DistributedStorage {
  ...
  async getBlobStream (org, name, dockerDigest) {
    const blobs = this.blobStore(org)
    if (blobs) {
      const ipfsCidNode = await blobs.get(dockerDigest)
      const ipfsCid = ipfsCidNode?.value
      ...
      // TODO: possibly write a transform/passthrough stream which calculates the digest as data flows through?
      return data
    }
    return null
  }
  ...
  async _putBlobStream (org, name, stream1, stream2) {
    // TODO: Ideally, we should just tee the stream here to calculate both the digest and add
    // to ipfs in parallel, but that seemed hard to do so just use two duplicate streams for now
    const digestPromise = new Promise((resolve, reject) =>
      stream1
        .on('error', reject)
        .pipe(crypto.createHash('sha256').setEncoding('hex'))
        .on('finish', function () {
          resolve(`sha256:${this.read()}`)
        }))

    // Add to IPFS and get CID.
    const ipfsAdded = await this.ipfs.add(stream2)
    const ipfsCid = ipfsAdded.cid.toString()
    debug(`stored blob in IPFS with CID ${ipfsCid} and digest ${await digestPromise}`)

    // Store the mapping between digest and cid in hyperdrive for that org.
    const blobs = this.blobStore(org)
    if (blobs) {
      await blobs.put(await digestPromise, ipfsCid)
    } else {
      throw new Error('tried to put blob into non-existing (thus not writable) org')
    }
  ...
  async patchUpload (org, name, uuid, data) {
    return this.uploadStore.appendUpload(uuid, data)
  }

  async putUpload (org, name, uuid, data) {
    const uploaded = await this.uploadStore.appendUpload(uuid, data)
    const [stream1, stream2] = await this.uploadStore.getUpload(uuid)
    const digest = await this._putBlobStream(org, name, stream1, stream2)
